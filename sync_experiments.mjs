...
async function sync() {
  const syncLockPath = path.join(WORKSPACE_DIR, "sync_experiments.lock");
  if (fs.existsSync(syncLockPath)) {
    // Check if the process is still running
    const oldPid = parseInt(fs.readFileSync(syncLockPath, "utf-8"));
    try {
      process.kill(oldPid, 0);
      console.log(`Sync already in progress (PID ${oldPid}), exiting.`);
      process.exit(0);
    } catch (e) {
      console.log(`Stale lock file found (PID ${oldPid}), removing.`);
      fs.unlinkSync(syncLockPath);
    }
  }
  fs.writeFileSync(syncLockPath, process.pid.toString());

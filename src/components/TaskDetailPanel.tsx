...
const TaskDetailPanel: React.FC<TaskDetailPanelProps> = ({
	...
	const lastUpdated = lastUpdatedActivity
		? lastUpdatedActivity._creationTime
		: null;
					...
					<div className="flex gap-2">
						{task.status !== "done" && task.status !== "archived" && (
							<button
								...
								title={
									!currentUserAgent ? "User agent not found" : "Mark as Done"
								}
							>
								<IconCheck size={16} />
								Mark as Done
						...
						{task.status !== "archived" && (
							<button
								...
								title={
									!currentUserAgent ? "User agent not found" : "Archive Task"
								}
							>
								<IconArchive size={16} />
								Archive
					...
					<div className="flex flex-wrap gap-1.5">
						...
						<div className="relative group">
							...
							<div className="absolute top-full left-0 mt-1 w-48 bg-white border border-border shadow-lg rounded-lg hidden group-hover:block z-10 p-1">
								...
								{agents?.filter((a) => !task.assigneeIds?.includes(a._id))
									.length === 0 && (

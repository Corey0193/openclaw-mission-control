...
function CtsBadge({ score }: { score: number | undefined | null }) {
	...
	return (
		<span
			className={`inline-flex items-center px-2.5 py-0.5 rounded-full border ${bg} ${color} text-xs font-black`}
		>
			{score}
		</span>
	);
...
export default function WalletsPage() {
	...
	const insiderWallets = useQuery(api.wallets.listInsiders, {
		tenantId: DEFAULT_TENANT_ID,
	});
	...
	const wallets = useMemo(() => {
		...
		const extraInsiders = insiders.filter(
			(w) => !paginatedAddrs.has(w.address),
		);
	...
	const SortIcon = ({ k }: { k: SortKey }) => {
		...
		if (sortDirection === "asc")
			return (
				<IconSortAscending size={14} className="text-[var(--accent-orange)]" />
			);
		return (
			<IconSortDescending size={14} className="text-[var(--accent-orange)]" />
		);
	};
									...
									<tr className="bg-muted/30 border-b border-border text-[11px] font-bold text-muted-foreground tracking-widest uppercase select-none">
										...
										>
											<div className="flex items-center gap-1">
												Wallet / User <SortIcon k="address" />
											</div>
										</th>
										<th
											className="text-left px-4 py-4 cursor-pointer hover:bg-muted/50 transition-colors group"
											onClick={() => handleSort("isInsider")}
										>
											<div className="flex items-center gap-1">
												Status <SortIcon k="isInsider" />
											</div>
										</th>
										<th
											className="text-right px-4 py-4 cursor-pointer hover:bg-muted/50 transition-colors group"
											onClick={() => handleSort("copyTradingScore")}
										>
											<div className="flex items-center justify-end gap-1">
												CTS <SortIcon k="copyTradingScore" />
											</div>
										</th>
										<th
											className="text-right px-4 py-4 cursor-pointer hover:bg-muted/50 transition-colors group"
										...
										>
											<div className="flex items-center justify-end gap-1">
												30d P&L <SortIcon k="pnl30d" />
											</div>
										</th>
										<th
											className="text-right px-4 py-4 cursor-pointer hover:bg-muted/50 transition-colors group"
											onClick={() => handleSort("computedWinRate")}
										>
											<div className="flex items-center justify-end gap-1">
												Win Rate <SortIcon k="computedWinRate" />
											</div>
										</th>
										<th
											className="text-right px-4 py-4 cursor-pointer hover:bg-muted/50 transition-colors group"
											...
											>
												...
												<td className="px-4 py-4 text-right">
													{(() => {
														...
														return (
															<span
																className={`font-bold ${wr >= 0.6 ? "text-emerald-600" : "text-foreground"}`}
															>
																{(wr * 100).toFixed(0)}%
															</span>
														);
												...
												<td className="px-4 py-4 text-right">
													<span className="font-bold">
														{w.tradeCount !== null && w.tradeCount !== undefined
															? w.tradeCount
															: "N/A"}
													</span>
												</td>
						...
						{wallets && (
							<div className="bg-muted/20 px-6 py-4 border-t border-border flex flex-col sm:flex-row items-center justify-between gap-4">
								<span className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest">
									Loaded {wallets.length} wallets (filtered:{" "}
								...
								{status === "LoadingMore" && (
									<div className="flex items-center gap-2 text-muted-foreground text-sm font-bold uppercase tracking-widest">
										<IconLoader2
											size={16}
											className="animate-spin text-[var(--accent-orange)]"
										/>
										Syncing...
									</div>
								)}

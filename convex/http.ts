...
http.route({
	...
	handler: httpAction(async (ctx, request) => {
		...
		await ctx.runMutation(api.arbPaperTrades.insertPaperTrade, {
			...
			makerExchange: body.maker_exchange ?? "",
			takerExchange: body.taker_exchange ?? "",
			polySide: body.poly_side ?? "",
			lmtsSide: body.lmts_side ?? "",
			makerPrice: body.maker_price ?? 0,
			takerPrice: body.taker_price ?? 0,
...
http.route({
	...
	handler: httpAction(async (ctx, request) => {
		...
		const count = await ctx.runMutation(api.arbPaperTrades.clearAllPaperTrades, {
			tenantId,
		});

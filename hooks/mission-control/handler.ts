...
const pendingWrites = new Map<
	string,
	{ filePath: string; content: string; sessionKey: string }
>();
...
function resolveUrl(cfg?: OpenClawConfig): string | undefined {
	...
	return (
		hookConfig?.env?.MISSION_CONTROL_URL || process.env.MISSION_CONTROL_URL
	);
}
 ...
 */
function extractCleanPrompt(rawPrompt: string): {
	prompt: string;
	source: string | null;
} {
	// Webchat metadata format:
	// "System: [timestamp] Node: hostname (IP) · app VERSION · mode MODE\n\nactual user message"
	const webchatMatch = rawPrompt.match(
		/^System:\s*\[\d{4}-[^\]]+\]\s*Node:\s*[^\n]+\n\n(.+)/s,
	);
	...
	const channelMatch = rawPrompt.match(
		/\[(?:Telegram|Discord|Slack|WhatsApp|SMS|Email)[^\]]+\]\s*(.+?)(?:\s*\[message_id:|$)/s,
	);
...
async function findAgentEventsModule(): Promise<{
	...
	if (
		g.__openclawAgentEvents &&
		typeof (g.__openclawAgentEvents as Record<string, unknown>).onAgentEvent ===
			"function"
	) {
		return g.__openclawAgentEvents as {
			onAgentEvent: (listener: (evt: AgentEventPayload) => void) => () => void;
		};
	}
	...
	if (mainPath) {
		...
		searchPaths.unshift(
			path.join(mainDir, "..", "dist", "infra", "agent-events.js"),
		);
	}
...
const handler = async (event: HookEvent) => {
	...
	console.log(
		`[mission-control] Event: ${event.type}:${event.action} session=${event.sessionKey}`,
	);
	...
	if (event.type === "agent" && event.action === "bootstrap") {
		...
		if (agentId && sessionId) {
			console.log(
				"[mission-control] Storing session info:",
				agentId,
				sessionId,
			);
		...
		try {
			...
			agentEvents.onAgentEvent(async (evt: AgentEventPayload) => {
					...
					if (phase === "start") {
						...
						if (info) {
							...
							if (rawPrompt) {
								...
								console.log(
									"[mission-control] Raw prompt:",
									rawPrompt.slice(0, 100),
								);
								console.log(
									"[mission-control] Clean prompt:",
									prompt.slice(0, 100),
								);
						...
						const isUserChannel =
							(messageChannel && userChannels.includes(messageChannel)) ||
							source !== null;

						if (
							!isUserChannel &&
							rawPrompt &&
							(rawPrompt.startsWith("System:") ||
								rawPrompt.startsWith("Read HEARTBEAT"))
						) {
							// System follow-up runs (exec notifications) — don't create new tasks,
							// but track them so tool events link to the original task
							console.log(
								"[mission-control] System follow-up run, linking to previous runId:",
								lastRealRunId.get(sessionKey),
							);
							return;
						}
						...
						console.log(
							"[mission-control] Tracked real runId:",
							evt.runId,
							"for session:",
							sessionKey,
						);
						...
						if (info) {
							...
							if (response) {
								...
								console.log(
									"[mission-control] Captured response:",
				...
				if (evt.stream === "tool") {
					...
					if (toolName && phase === "start") {
						...
						if (toolName === "write" && toolCallId) {
							const args = evt.data?.args as
								| Record<string, unknown>
								| undefined;
							const filePath = (args?.file_path ?? args?.path) as
								| string
								| undefined;
							const content = args?.content as string | undefined;

							if (filePath && content) {
								pendingWrites.set(toolCallId, {
									filePath,
									content,
									sessionKey,
								});
								console.log(
									`[mission-control] Tracking write: ${toolCallId} -> ${filePath}`,
								);
							}
						}
					}
					...
					if (toolName === "write" && phase === "result" && toolCallId) {
						...
						if (pending && !isError) {
							...
							console.log(
								`[mission-control] Document captured: ${fileName} (${docType})`,
							);
						}
					...
					if (
						...
						const rawResult = evt.data?.result as
							| string
						...
						if (text) {
							...
							const fileMatch = text.match(
								/(\/\S+\.(?:png|jpg|jpeg|gif|webp|svg|mp4|mp3|wav|pdf))/i,
							);
							...
							if (fileMatch && fileMatch[1]) {
								...
								console.log(
									`[mission-control] Exec file captured: ${fileName} (${docType})`,
								);
							} else {
								console.log(
									`[mission-control] Exec result (no file found): ${text.slice(0, 150)}`,
								);
							}
						}
					}
				...
				if (!["lifecycle", "tool", "exec", "assistant"].includes(evt.stream)) {
					console.log(
						`[mission-control] Unhandled stream: ${evt.stream}`,
